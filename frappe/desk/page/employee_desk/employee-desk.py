...
from frappe.utils import today, time_diff_in_hours, date_diff, now, nowdate, flt, getdate, cint
...
def get_employee_info(user=None, checkin_type=None, half_day=None):
	...
	emp = frappe.db.sql("""
    			...
    			ifnull((select b.shift_type from `tabAssign Shift` b, `tabShift Details` sd, `tabShift Type` st where sd.parent = b.name and sd.employee = e.employee and b.shift_type = st.name and '{0}' between st.start_time and st.end_time and '{1}' between b.from_date and b.to_date and sd.{2} = 1),e.default_shift) as shift_type,
...
def make_employee_checkin(employee, employee_name, shift_type, time, time_difference, reason=None, checkin_type = None):
	if checkin_type != "":
		...
		if ct[0] == "Lunch" and ct[1] == "OUT":
			...
			if datetime.strptime(str(time),"%H:%M:%S") < datetime.strptime(str(future_date_and_time),"%H:%M:%S"):
...
def get_job_description(user):
	data = []
	emp = frappe.db.sql("""
                          select name, branch, designation from `tabEmployee` where user_id = '{}'
	...
	if "Branch Manager" in frappe.get_roles(user):
		jd = frappe.db.sql("""
            select attachment from `tabJob Description` where particular = '{0}'
                     """.format(emp[0].branch), as_dict=True)
	else:
		jd = frappe.db.sql("""
            select attachment from `tabJob Description` where particular = '{0}'
                     """.format(emp[0].designation), as_dict=True)
...
def get_emp_info(user):
	data = []
	info = frappe.db.sql("""
				select name, grade, designation 
				from `tabEmployee`
				where user_id = "{}"
	""".format(user), as_dict=True)
	if info:
		info = info[0]
		data.append({"name": info.name, "grade": info.grade, "designation": info.designation})
	return info
...
def get_pms_records(user):
	...
	pms_records = frappe.db.sql("""
        select fiscal_year, final_score, overall_rating from `tabEmployee PMS Rating` where parent = '{0}' order by fiscal_year desc
...
def get_emp_work_history(user):
	...
	history = frappe.db.sql("""
        select reference_doctype, reference_docname, branch, department, division, designation, from_date, to_date, promotion_due_date from `tabEmployee Internal Work History` where parent = '{0}'
        order by reference_doctype desc
                             """.format(employee), as_dict=True)
